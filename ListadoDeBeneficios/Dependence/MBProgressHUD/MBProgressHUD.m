...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.opacity = 1.0f;
        self.color =[UIColor colorWithHexString:@"ff6702"];
...
- (void)drawRect:(CGRect)rect {
	...
	CGRect boxRect = CGRectMake(round((allRect.size.width - 28) / 2) + self.xOffset,
								round((allRect.size.height - 28) / 2) + self.yOffset, 28, 28);
	float radius = 14;
...
CGContextSetRGBStrokeColor(context, 255.0/255.0, 255.0/255.0, 255.0/255.0, 1.0);//[UIColor colorWithHexString:@"ff6702"]
     CGContextStrokeEllipseInRect(context, boxRect);
